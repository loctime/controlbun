...
Automatiza la subida de documentos PDF a controldocumentario.com usando Codex/Gemini para el matching visual y Playwright para la automatización web. Cada cliente es un usuario de Telegram con sus propias credenciales de CD y sus propios mapeos.
...
| `Codex.js` | Multi-provider AI: Codex Haiku / Gemini 2.5 Flash / Ollama. Matching de páginas (`matchearPaginasConReqs`). Provider switcheable en runtime. |
...
| `/modelo` | admin | Cambiar provider de AI en runtime: `/modelo Codex` o `/modelo gemini` |
...
- `/modelo Codex` o `/modelo haiku` → cambia a Codex Haiku
...
`Codex.js` lee `AI_PROVIDER` del `.env` al arrancar, pero puede sobreescribirse con `runtime.json` (persiste entre reinicios) o con el comando `/modelo` (cambia en memoria + escribe `runtime.json`).
- **Producción**: Codex Haiku (`Codex-haiku-4-5-20251001`) — más robusto para JSON estructurado
...
Codex/Gemini ve TODAS las refs + TODAS las páginas nuevas en una sola llamada.
...
### matchearPaginasConReqs — lógica de validación y deduplicación (Codex.js)
...
`Codex.js` soporta tres providers, controlados por `AI_PROVIDER` en `.env` (o sobreescrito por `runtime.json`):

| Valor | Descripción |
|---|---|
| `Codex` | Producción — Anthropic API, modelo `Codex-haiku-4-5-20251001` |
...
| Codex.js: multi-provider (Codex/Gemini/Ollama) | ✅ Switcheable en runtime |
| Codex.js: matchearPaginasConReqs | ✅ Filtros de entidad y nombre, rescue entity-mismatch, docs empresa sin entidad, nombre empresa en prompt |
...
- No agregar chain-of-thought al prompt de Codex/Gemini para matching
